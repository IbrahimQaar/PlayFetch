  ...
  return (
    <div className= {styles.viewLogin}>
      ...
      <label htmlFor="username">Username:</label>
          <input type="text" className= {styles.username} value={username} size="50" required autoFocus autoComplete="username"
              onChange={ event => setUsername(event.target.value)} />
        </div>

        <div className= {styles.formControl}>
        <label htmlFor="password">Password:</label>
          <input type="password" className= {styles.password} value={password} size="50" required
              onChange={ event => setPassword(event.target.value)} />
        </div>

        <button type="submit" className={styles.btnPrimary}>Sign in</button>
        <Link to="/register" className={styles.register}>New? Register here!</Link>
      </form>
    </div>
  );
}
